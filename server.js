...
function gateKeeper(req, res, next) {
  ...
  const credentials = req.get('x-username-and-password'); //user=joeschmoe@business.com&pass=password
  const parsedCredentials = queryString.parse(credentials) //{user: 'joefdfmsfd', password: 'Pass'}
  req.user = USERS.find(function(user) {
    return user.userName === parsedCredentials.user && user.password === parsedCredentials.pass
  })
  next();
}
